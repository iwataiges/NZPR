...
def plot(df1, df2, df3, year1_list, year2_list, df_sep):
    ...
    for i in range(n_subcat_to_plot):
        ...
        # SEP
        for j in range(6):
            tdx = df_sep[(df_sep['Scenario']==j) & (df_sep['id']==list_subcategory[idx]) & (df_sep['Type']=='FEC(energy-related)')]
            tx = tdx['Year']
            ty = tdx['Value']/1.0e6
            if j < 5:
                marker = 'o'
        ...
        plt.tight_layout()
        #plt.show()
        outputfilename = 'charts/20260205_10_plot_energy_%02d_%s.png' % (i, list_subcatlabel[idx])
