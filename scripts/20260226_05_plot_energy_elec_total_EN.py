...
jsonfile1_data = 'outputs/20251201_21_energy_stat/20251201_21_energy_stat_data_common_0_電力.json'
jsonfile2_data = 'outputs/20251201_01_1p5CRM_balance_energy/20251201_11_1p5CRM_balance_energy_data_common_07_電力.json'
jsonfile3_data = 'outputs/20251201_03_1p5CRM_steps_energy/20251201_13_1p5CRM_steps_energy_data_common_07_電力.json'
...
def plot(df1, df2, df3, year1_list, year2_list, df_sep):
    ...
    ax.set_yticks(np.arange(ymin, ymax, step=0.5))

    ymin2 = ymin * 1.0e6 / 3600 # EJ to TWh
    ymax2 = ymax * 1.0e6 / 3600 # EJ to TWh
    ax2.set(ylim=(ymin2, ymax2))
    ax2.set_yticks(np.arange(600, 1200, step=100))
    ...
    list_years = []
    for j in range(FIT_YEAR_START, FIT_YEAR_END):
    ...
    # SEP
    for j in range(6):
        tdx = df_sep[(df_sep['Scenario']==j) & (df_sep['Sector']==sector_str[0]) & (df_sep['Type']=='Electricity Demand')]
        tx = tdx['Year']
        ty = tdx['Value']*3600/1.0e6 # TWh to EJ
        if j < 5:
            marker = 'o'
...
#    ax.set_title(list_subcatlabel[0])
    ...
    ax.text(xmax - (xmax-xmin)*0.13, ymax - (ymax-ymin)*0.07, 'Power', color=config.COL_ASBESTOS_DARK, fontsize=24)

 
    #ax.legend(loc='lower left')
    plt.tight_layout()
    #plt.show()
    plt.savefig('charts/20260226_05_plot_energy_elec_total_EN.png')
