...
def plot(df1, df2, df3, year1_list, year2_list, df_sep):
    ...
    for i in range(n_subcategory):
        ...
        if sector_str[i] != "":
            for j in range(6):
                tdx = df_sep[(df_sep['Scenario']==j) & (df_sep['Sector']==sector_str[i]) & (df_sep['Type']=='CO2 Intensity(energy-related)')]
                tx = tdx['Year']
                ty = tdx['Value']
                if j < 5:
                    marker = 'o'
    ...
    plt.tight_layout()
    #plt.show()
    plt.savefig('charts/20260226_24_plot_intensity_subcat_energy.png')
