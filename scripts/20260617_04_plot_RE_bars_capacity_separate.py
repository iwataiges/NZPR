...
INPUT_EXCEL2 = 'inputs/RE/20260618_06_RE_cap_for_plot.xlsx'
OUTPUT_PLOT = 'charts/20260617_04_plot_RE_bars_capacity_separate.png'
...
def plot_sub(ax, df1, df2, label, ymax, pos, title):
    xmin = -5
    xmax = 35
    width1 = 0.5
    fs = 14
    ...
    for i in range(ndata1):
        ...
        x = year - year0
        value = df1.iloc[i][label+'_GW_IRENA']
...
#            x = df2.iloc[i]['x'] * factor
            ...
            value = df2.iloc[i][label]
            colpos = df2.iloc[i]['Tone']

            if gradflg:
                value_high = df2.iloc[i][label + '_high']
...
def plot_fit_line(ax, df1, df2, label, array, pos):
    year0 = 2010
    x0 = 2019
    x1 = 2024
    x2 = 2040
    ...
    if len(array) > 1:
        a1 = array[1]
        b = df1[df1['Year']==x1][label+'_GW_IRENA']
    ...
    a0 = array[0]
    b = df1[df1['Year']==x0][label+'_GW_IRENA']
...
def plot(df1, df2):
    ...
    plot_sub(axs[0], df1, df2, 'PV', 300, 0, '太陽光')
    plot_sub(axs[1], df1, df2, 'OnSW', 150, 1, '陸上風力')
    plot_sub(axs[2], df1, df2, 'OffSW', 150, 2, '洋上風力')

    plot_fit_line(axs[0], df1, df2, 'PV', [5.282, 2.533], 0)
    plot_fit_line(axs[1], df1, df2, 'OnSW', [0.338], 1)
